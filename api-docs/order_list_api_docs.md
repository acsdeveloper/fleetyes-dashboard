...
| `driver_assigned` | `{ uuid, name }` \| null | Assigned driver |
| `vehicle_assigned` | `{ uuid, name }` \| null | Assigned vehicle (`name` is computed from year/make/model/trim/plate) |
...
| `payload` | object \| null | Payload with pickup/dropoff place details. View API also includes `waypoints` |
...
|---|---|---|
| Pagination | Yes (limit/page or all/batch_page) | Single record |
| `payload.waypoints` | Not included | Included |

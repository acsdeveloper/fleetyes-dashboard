...
> Coordinates are stored as `[longitude, latitude]` (GeoJSON standard — longitude first).
...
| `location` | GeoJSON | No | `{ type: "Point", coordinates: [lng, lat] }` |

  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-6 md:p-8 lg:p-10">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
        <div><PageHeader pageKey="tollExpenses" /><p className="mt-1 text-sm text-muted-foreground">Track road tolls, bridge, and tunnel charges across the fleet.</p></div>
        <div className="flex gap-2">
          ...
          <button className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-primary px-3 text-sm font-medium text-primary-foreground hover:bg-primary/90"><Plus className="h-3.5 w-3.5" /> New Charge</button>
        </div>
      </div>

      <div className="grid gap-4 sm:grid-cols-4">
        {[
          { label:"Total (shown)",      value:`£${total.toFixed(2)}`                                },
          { label:"Total This Month",   value:`£${tollExpenses.reduce((a,r)=>a+r.amount,0).toFixed(2)}` },
          { label:"Pending Reconcile",  value:tollExpenses.filter(r=>r.status==="pending").length   },
          { label:"OBU Charges",        value:tollExpenses.filter(r=>r.method==="OBU").length        },
        ].map(k=>(
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 max-w-xs">
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search by reg, driver, route…" className="h-9 w-full rounded-lg border bg-background pl-9 pr-3 text-sm outline-none focus:ring-2 focus:ring-ring" />
        </div>
      ...
      <div className="overflow-auto rounded-xl border bg-card shadow-sm">
        <table className="w-full text-sm">
          <thead><tr className="border-b bg-muted/40">{["Ref","Date","Vehicle","Driver","Route","Type","Amount","Method","Status","Action"].map(h=><th key={h} className="px-4 py-2.5 text-left text-xs font-medium text-muted-foreground whitespace-nowrap">{h}</th>)}</tr></thead>
          <tbody>
            {filtered.map(r=>(
              <tr key={r.id} className="border-b last:border-0 hover:bg-muted/20">
                ...
                <td className="px-4 py-2.5"><button className="text-xs text-indigo-500 hover:underline">{r.status==="pending"?"Reconcile":"View"}</button></td>
              </tr>
            ))}
          </tbody>
          <tfoot><tr className="border-t bg-muted/20"><td colSpan={10} className="px-4 py-2 text-xs text-muted-foreground">{filtered.length} charges · £{total.toFixed(2)}</td></tr></tfoot>
        </table>
      </div>
    </div>
  )

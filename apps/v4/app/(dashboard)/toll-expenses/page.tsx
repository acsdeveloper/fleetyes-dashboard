  ...
  const colDefs = React.useMemo<ColDef<TollReport>[]>(() => [
    ...
    { headerName: "Incl. VAT",       valueGetter: ({ data }) => data?.amount_incl_tax ?? "", filter: "agNumberColumnFilter", width: 100, cellRenderer: ({ data }: ICellRendererParams<TollReport>) => <span className="text-xs text-muted-foreground">{data?.amount_incl_tax ? `${data.currency} ${Number(data.amount_incl_tax).toFixed(2)}` : "—"}</span> },
    {
      ...
      cellRenderer: ({ value }: ICellRendererParams) => {
        ...
        return <span className={`inline-flex items-center rounded-[100px] border pl-1 pr-3 text-[12px] font-medium capitalize leading-[2] ${s.bg} ${s.border} ${s.text}`}><span className={`mr-2 ml-1.5 inline-block h-2 w-2 shrink-0 rounded-full ${s.dot}`} />{AMAZON_LABELS[value] ?? value}</span>
      },
    },
    {
      ...
      <div data-help="toolbar" className="flex flex-col gap-2">
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1 rounded-lg border bg-muted/40 p-0.5">
            <span className="rounded-md px-3.5 py-1.5 text-sm font-medium bg-card shadow-sm text-foreground">{t.nav.tollExpenses}</span>
          </div>

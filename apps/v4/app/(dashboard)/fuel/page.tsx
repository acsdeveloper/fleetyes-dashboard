...
// ─── Expense status styles ────────────────────────────────────────────────────

const EXP_STATUS: Record<string, { bg: string; border: string; text: string; dot: string }> = {
  pending:  { bg: "bg-amber-50 dark:bg-amber-900/20",   border: "border-amber-300/70",   text: "text-amber-800 dark:text-amber-300",   dot: "bg-amber-500" },
...
function ReceiptDetailDrawer({ receipt, onClose }: { receipt: FuelReceiptImage; onClose: () => void }) {
    ...
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-50 flex w-full max-w-2xl flex-col border-l bg-card shadow-2xl">
        <div className="flex items-center justify-between border-b px-5 py-4">
          <div>
            <h2 className="text-base font-bold">Fuel Receipt Detail</h2>
...
// ─── Upload Wizard (auto-processes after import) ──────────────────────────────

type UploadStep = "upload" | "uploading" | "importing" | "processing" | "done" | "error"

function ReceiptUploadWizard({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
      ...
      <div className="fixed inset-y-0 right-0 z-50 flex w-full max-w-lg flex-col border-l bg-card shadow-2xl">
        <div className="flex items-center justify-between border-b px-5 py-4">
          <div>
            ...
            <p className="mt-0.5 text-xs text-muted-foreground">Upload a ZIP of receipt images — OCR runs automatically</p>
          </div>
          <button onClick={onClose} className="rounded-lg border p-1.5 text-muted-foreground hover:bg-muted"><X className="h-4 w-4" /></button>
        </div>
        ...
        <div className="flex flex-1 flex-col gap-6 overflow-y-auto p-5">
          {step === "upload" && (
            <>
              <div className="rounded-xl border bg-muted/30 p-4 text-xs">
                <p className="mb-2 font-semibold uppercase tracking-wider text-muted-foreground">Format</p>
                <p className="text-muted-foreground">Upload a <strong>.zip</strong> of JPEG/PNG receipt images. The system will OCR each image and extract fuel details automatically — no manual processing step required.</p>
              </div>
          ...
          {(step === "uploading" || step === "importing" || step === "processing") && (
            <div className="flex flex-col gap-4 py-6">
                ...
                {STEPS.map((s, i) => (
                  <React.Fragment key={s.key}>
                    <div className={`flex items-center gap-1.5 ${i <= stepIdx ? "text-primary" : "text-muted-foreground/40"}`}>
                      <span className={`flex h-5 w-5 items-center justify-center rounded-full text-[10px] font-bold ${i < stepIdx ? "bg-green-500 text-white" : i === stepIdx ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>
                        {i < stepIdx ? "✓" : i + 1}
                      </span>
                      <span className="text-xs font-medium">{s.label}</span>
                    </div>
                    {i < STEPS.length - 1 && <div className={`h-px flex-1 ${i < stepIdx ? "bg-green-400" : "bg-border"}`} />}
                  </React.Fragment>
                ))}
              </div>
              <div className="flex flex-col items-center gap-3 py-8">
                ...
                <p className="font-medium">
                  {step === "uploading" ? "Uploading ZIP file…" : step === "importing" ? "Importing receipt records…" : "Running OCR processing…"}
                </p>
                <p className="text-xs text-muted-foreground">
                  {step === "processing" ? "This may take a moment for large batches" : "Please wait…"}
                </p>
              </div>
            </div>
          )}
            ...
            <div className="flex flex-col gap-4">
              <div className="flex items-start gap-3 rounded-xl bg-green-50 dark:bg-green-950/20 p-4">
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-muted-foreground">{importedCount} receipts imported</p>
        ...
        <div className="flex gap-2 border-t p-4">
          {step === "upload" && (
            <>
              ...
              <button onClick={runUpload} disabled={!file} className="flex-1 rounded-lg bg-primary px-3 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50">
                Upload & Process
              </button>
            </>
          )}
...
type ExpFilters = { vehicle: string; status: string; payment_method: string; from_date: string; to_date: string }
const EMPTY_EXP_FILTERS: ExpFilters = { vehicle: "", status: "", payment_method: "", from_date: "", to_date: "" }
...
function ReceiptsTab({ isDark, drivers }: { isDark: boolean; drivers: Driver[] }) {
  ...
  const colDefs = React.useMemo<ColDef<FuelReceiptImage>[]>(() => [
    {
      colId: "_view", headerName: "", width: 44, minWidth: 44, maxWidth: 44,
    ...
    { headerName: "Receipt Date", valueGetter: ({ data }) => data?.extracted_data?.date ?? "",                         filter: "agTextColumnFilter", width: 110, cellRenderer: ({ value }: ICellRendererParams) => <span className="text-xs text-muted-foreground">{value || "—"}</span> },
    ...
    { headerName: "VRN",          valueGetter: ({ data }) => data?.extracted_data?.vehicle_vrn ?? "",                   filter: "agTextColumnFilter", width: 100, cellRenderer: ({ value }: ICellRendererParams) => <span className="font-mono text-xs">{value || "—"}</span> },
    {
      headerName: "Status", field: "status", filter: "agTextColumnFilter", width: 120,
      cellRenderer: ({ value }: ICellRendererParams) => {
        const s = REC_STATUS[value] ?? REC_STATUS.pending
        return value ? <span className={`inline-flex items-center rounded-[100px] border pl-1 pr-3 text-[12px] font-medium capitalize leading-[2] ${s.bg} ${s.border} ${s.text}`}><span className={`mr-2 ml-1.5 inline-block h-2 w-2 shrink-0 rounded-full ${s.dot}`} />{value}</span> : null
      },
    },
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ], [])
    ...
    <div className="flex flex-1 flex-col gap-3 overflow-hidden">
      ...
      {showCards && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            { label: "Total Receipts", value: meta.total,      colour: "text-blue-600 bg-blue-50 dark:text-blue-400 dark:bg-blue-900/20" },
            ...
            { label: "Processed",      value: processedCount,  colour: "text-emerald-600 bg-emerald-50 dark:text-emerald-400 dark:bg-emerald-900/20" },
            { label: "Failed",         value: failedCount,     colour: "text-red-600 bg-red-50 dark:text-red-400 dark:bg-red-900/20",            alert: failedCount > 0 },
          ].map(k => (
            <div key={k.label} className={`relative flex flex-col gap-2 rounded-xl border bg-card px-4 py-3 shadow-sm ${"alert" in k && k.alert ? "border-amber-300 dark:border-amber-700" : ""}`}>
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-medium text-muted-foreground">{k.label}</span>
                <span className={`rounded-lg p-1.5 ${k.colour}`}>
              ...
              {"alert" in k && k.alert && <span className="absolute right-3 top-3 h-2 w-2 rounded-full bg-amber-500" />}
            </div>
          ))}
        </div>
      )}
      ...
      <div className="relative flex-1 overflow-hidden rounded-xl border bg-card shadow-sm">
        ...
        {!loading && records.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center gap-2 text-sm text-muted-foreground">
            ...
            <p className="text-xs">Receipts sync from the driver app, or upload a ZIP</p>
          </div>
        ) : (
      ...
      {meta.last_page > 1 && (
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          ...
          <div className="flex items-center gap-1">
            <button disabled={page <= 1} onClick={() => setPage(p => p - 1)} className="rounded px-2 py-1 hover:bg-muted disabled:opacity-40">←</button>
            <span>Page {page} / {meta.last_page}</span>
            <button disabled={page >= meta.last_page} onClick={() => setPage(p => p + 1)} className="rounded px-2 py-1 hover:bg-muted disabled:opacity-40">→</button>
          </div>
        </div>
      )}

      {detailRecord && <ReceiptDetailDrawer receipt={detailRecord} onClose={() => setDetailRecord(null)} />}
      {showUpload && <ReceiptUploadWizard onClose={() => setShowUpload(false)} onDone={() => fetchData(1)} />}
      <ReceiptFilterDrawer open={showFilter} onClose={() => setShowFilter(false)} filters={filters} setFilters={setFilters} drivers={drivers} />
    </div>
  )
}
...
function ExpensesTab({ isDark, vehicles, drivers }: { isDark: boolean; vehicles: Vehicle[]; drivers: Driver[] }) {
  ...
  const handleDelete = async (uuid: string) => {
    const ok = await confirm({ title: "Delete fuel expense", description: "This will permanently remove the fuel expense record." })
    if (!ok) return
  ...
  const handleBulkDelete = async () => {
    ...
    const ok = await confirm({ title: `Delete ${selected.length} record${selected.length !== 1 ? "s" : ""}`, description: "This action is permanent and cannot be undone." })
    if (!ok) return
    try { await bulkDeleteFuelExpenses(selected.map(r => r.uuid)); fetchData(page) }
    catch (e: unknown) { alert(e instanceof Error ? e.message : "Delete failed") }
  }
  ...
  const colDefs = React.useMemo<ColDef<FuelExpense>[]>(() => [
    ...
    { headerName: "Payment",  field: "payment_method", filter: "agTextColumnFilter", width: 110, cellRenderer: ({ value }: ICellRendererParams) => <span className="text-xs text-muted-foreground">{value || "—"}</span> },
    {
      headerName: "Status", field: "status", filter: "agTextColumnFilter", width: 120,
      cellRenderer: ({ value }: ICellRendererParams) => {
        const s = EXP_STATUS[value] ?? EXP_STATUS.pending
        return value ? <span className={`inline-flex items-center rounded-[100px] border pl-1 pr-3 text-[12px] font-medium capitalize leading-[2] ${s.bg} ${s.border} ${s.text}`}><span className={`mr-2 ml-1.5 inline-block h-2 w-2 shrink-0 rounded-full ${s.dot}`} />{value}</span> : null
      },
    },
    {
  ...
  return (
    <div className="flex flex-1 flex-col gap-3 overflow-hidden">
      ...
      {showCards && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            { label: "Total Records",    value: meta.total,                                               sub: "all time",       colour: "text-blue-600 bg-blue-50 dark:text-blue-400 dark:bg-blue-900/20" },
            { label: "Cost (this page)", value: `£${records.reduce((a, r) => a + Number(r.amount ?? 0), 0).toFixed(2)}`, sub: "excl. VAT", colour: "text-emerald-600 bg-emerald-50 dark:text-emerald-400 dark:bg-emerald-900/20" },
            { label: "Pending Approval", value: records.filter(r => r.status === "pending").length,       sub: "awaiting review", colour: "text-amber-600 bg-amber-50 dark:text-amber-400 dark:bg-amber-900/20" },
            { label: "Approved",         value: records.filter(r => r.status === "approved").length,      sub: "this page",       colour: "text-violet-600 bg-violet-50 dark:text-violet-400 dark:bg-violet-900/20" },
          ].map(k => (
            <div key={k.label} className="relative flex flex-col gap-2 rounded-xl border bg-card px-4 py-3 shadow-sm">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-medium text-muted-foreground">{k.label}</span>
                <span className={`rounded-lg p-1.5 ${k.colour}`}>
      ...
      <div className="flex items-center gap-2">
        {selectedCount > 0 && (
          <button onClick={handleBulkDelete}
            className="inline-flex h-8 items-center gap-1.5 rounded-lg bg-red-500 px-3 text-xs font-semibold text-white shadow-sm hover:bg-red-600">
      ...
      <ExpenseFilterDrawer open={showFilter} onClose={() => setShowFilter(false)} filters={filters} setFilters={setFilters} vehicles={vehicles} />
    </div>
  )
}
...
type FuelTab = "receipts" | "expenses"

export default function FuelPage() {
  const [activeTab, setActiveTab] = React.useState<FuelTab>("receipts")
  ...
  const tabs: { key: FuelTab; label: string; icon: React.ReactNode }[] = [
    { key: "receipts", label: "Receipts",  icon: <Receipt className="h-3.5 w-3.5" /> },
    { key: "expenses", label: "Expenses",  icon: <CreditCard className="h-3.5 w-3.5" /> },
  ]
  ...
  return (
    <div className="flex flex-1 flex-col gap-3 overflow-hidden px-6 pt-3 pb-2 md:px-8 lg:px-10">
      ...
      {/* Tab content */}
      {activeTab === "receipts" && <ReceiptsTab isDark={isDark} drivers={drivers} />}
      {activeTab === "expenses" && <ExpensesTab isDark={isDark} vehicles={vehicles} drivers={drivers} />}
    </div>
  )
}
